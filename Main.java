...
class User{
	...
	public void getUserInfo(){
		...
		System.out.println("");
		System.out.println("Name detials:");
		System.out.println("");
	...
	public boolean checkValidity(){
		...
		try {
			...
			if(medication_history==0) {
				...
				System.out.println("");
				System.out.println("If yes ,please specify :1)Medications for skincare and similar reasons "
						+ "\n 2)Medications for vitamin deficiency and more");
...
//check the validity of the donor
		if(checkValidity()) {
			//accessing the donor details from the parent class
...
//method to send donor first in line to blood donation center
	public String dequeue() {
		...
		int size_of_address=temp.house_addr.length();

		int index = 0;


		// Loop to traverse the string until a space is encountered

		while (index < temp.house_addr.length() && temp.house_addr.charAt(index) != ' ') {

			char currentChar = temp.house_addr.charAt(index);

			pin_code+=currentChar;

			index++;

}

		System.out.println("The donor "+temp.name+" can now donate blood in blood bank "+pin_code);
//to check id donor queue is empty
		if (isEmpty()) {
